...
func (blockChain *BlockChain) AddBlock(txs []*Transaction) {
	...
	blockChain.boltDB.Update(func(tx *bolt.Tx) error {
		...
		// 添加区块
		block := NewBlock(txs, blockChain.lastBlockHash)
		err := bucket.Put(block.Hash, block.ToBytes())
		// 存入最后一个区块 hash
		err = bucket.Put([]byte(LastHashKey), block.Hash)
...
// 查找 UTXO
func (blockChain *BlockChain) FindMyUTXOs(address string) []UTXOInfo {
...
func (blockChain *BlockChain) GetBalance(address string) {
	UTXOInfos := blockChain.FindMyUTXOs(address)
	total := 0.0
...
// 遍历账本，找到属于付款人的合适金额
func (blockChain *BlockChain) FindNeedUTXOs(address string, amount float64) (map[string][]int, float64) {
